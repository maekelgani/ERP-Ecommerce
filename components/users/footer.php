...
function formatWilayah(?string $text): string
{
    if (!$text) return '';
...
function formatSiteTitle(string $title): string
{
            ...
            <div class="grid grid-cols-1 gap-12 lg:grid-cols-12">
                ...
                <div class="lg:col-span-4 space-y-6">
                    ...
                    <div class="flex items-center gap-3 group cursor-pointer">
                        <div class="relative">
                            ...
                            <img alt="<?= !empty($globalSettings['site_title']) ? htmlspecialchars($globalSettings['site_title']) : 'Nano Komputer' ?> Logo"
                                src="<?= !empty($globalSettings['site_logo']) ? '../../' . htmlspecialchars($globalSettings['site_logo']) : '../../assets/img/mainicon.png' ?>"
                    ...
                    <p class="text-white/70 leading-relaxed text-sm max-w-sm">
                        <?= !empty($globalSettings['site_description']) ? htmlspecialchars($globalSettings['site_description']) : 'Di Nano Komputer, kami memahami pentingnya kualitas dan keamanan dalam setiap pembelian.
                    ...
                    <div class="pt-4">
                        ...
                        <div class="flex gap-3">
                            <?php if (!empty($globalSettings['facebook_url'])): ?>
                                <a href="<?= htmlspecialchars($globalSettings['facebook_url']) ?>" rel="noreferrer" target="_blank"
                            ...
                            <?php if (!empty($globalSettings['instagram_url'])): ?>
                                <a href="<?= htmlspecialchars($globalSettings['instagram_url']) ?>" rel="noreferrer" target="_blank"
                            ...
                            <?php if (!empty($globalSettings['youtube_url'])): ?>
                                <a href="<?= htmlspecialchars($globalSettings['youtube_url']) ?>" rel="noreferrer" target="_blank"
                            ...
                            <?php endif; ?>
                            <?php if (!empty($globalSettings['tiktok_url'])): ?>
                                <a href="<?= htmlspecialchars($globalSettings['tiktok_url']) ?>" rel="noreferrer" target="_blank"
                            ...
                            <ul class="space-y-5">
                                <li>
                                    <?php if (!empty($globalSettings['contact_email'])): ?>
                                        <a href="mailto:<?= htmlspecialchars($globalSettings['contact_email']) ?>" class="group flex items-start gap-3 text-white/60 hover:text-white transition-all duration-300">
                                            ...
                                            <div>
                                                <?php if (!empty($globalSettings['contact_email'])): ?>
                                                    ...
                                                    <span class="text-sm"><?= htmlspecialchars($globalSettings['contact_email']) ?></span>
                                                <?php else: ?>
                                                    <p class="text-xs text-white/40 mb-0.5">Email</p>
                                                    <span class="text-sm">cs.email@youremail.com</span>
                                                <?php endif; ?>
                                            </div>
                                        </a>
                                    <?php endif; ?>
                                ...
                                <li>
                                    <?php if (!empty($globalSettings['contact_phone'])):
                                        $displayPhone = $globalSettings['contact_phone'];

                                        // 1. Bersihkan karakter non-angka (spasi, strip, kurung)
                                        $cleanPhone = preg_replace('/[^0-9]/', '', $displayPhone);

                                        // 2. Ubah 08xxx menjadi 628xxx (Format Internasional WA)
                                    ...
                                    ?>
                                        <a href="<?= $waUrl ?>" target="_blank" rel="noopener noreferrer"
                                            class="group flex items-start gap-3 text-white/60 hover:text-white transition-all duration-300 mb-4">

                                            <div class="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0 group-hover:bg-green-500/20 group-hover:text-green-400 transition-colors">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <div>
                                                <p class="text-xs text-white/40 mb-0.5 font-medium uppercase tracking-wide group-hover:text-white/60">WhatsApp / Telepon</p>
                                                <span class="text-sm font-semibold tracking-wide group-hover:underline decoration-white/30 underline-offset-4">
                                ...
                                <li class="flex items-start gap-3 text-white/60">
                                    ...
                                    <div>
                                        ...
                                        ?>
                                        <p class="text-xs text-white/40 mb-0.5 uppercase tracking-wider font-semibold">Alamat Toko</p>
                                        <address class="text-sm not-italic leading-relaxed">
                                            <?php if ($primaryStore): ?>
                                                <?= htmlspecialchars($primaryStore['alamat']) ?>,
                                                <?= htmlspecialchars($primaryStore['kelurahan'] ?? '') ?>,
                                                <?= htmlspecialchars($primaryStore['kecamatan'] ?? '') ?>,
                                                <?= htmlspecialchars($primaryStore['kota_kabupaten'] ?? '') ?>,
                                                <?= htmlspecialchars($primaryStore['provinsi'] ?? '') ?>
                                        ...
                                        <p class="text-xs text-white/40 mt-2 flex items-center gap-1.5">
                                            ...
                                            <?php if ($primaryStore && !empty($primaryStore['jam_buka']) && !empty($primaryStore['jam_tutup'])): ?>
                                                Setiap Hari: <?= substr($primaryStore['jam_buka'], 0, 5) ?> - <?= substr($primaryStore['jam_tutup'], 0, 5) ?>
                                            <?php else: ?>
                    ...
                    <p class="text-white/50 text-sm text-center md:text-left">
                        ...
                        <span class="text-white/70 font-medium">
                            <?= !empty($globalSettings['site_title'])
                                ? htmlspecialchars(mb_convert_case(formatSiteTitle($globalSettings['site_title']), MB_CASE_TITLE, 'UTF-8'))
                                : 'Nano Komputer'
                            ?>.
                        </span>
                        All rights reserved.
