                    ...
                    <?php if (!empty($bestSellers)): ?>
                        <ul class="mt-8 grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                            <?php foreach ($bestSellers as $product): ?>
                                ...
                                $price = $productHelper->formatPrice($product['harga']);
                                ...
                                <li class="group bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col h-full overflow-hidden">
                                    ...
                                    <div class="p-4 sm:p-5 flex flex-col justify-between flex-grow">
                                        ...
                                        <div class="mt-4 flex gap-2">
                                            ...
                                            <button class="w-10 flex-shrink-0 rounded-lg bg-gray-100 px-2.5 py-2.5 text-sm font-medium text-gray-600 transition-all hover:bg-gray-50 hover:border-[#882426] hover:text-[#882426] <?= !$stockBadge['available'] ? 'opacity-50 cursor-not-allowed' : '' ?>" <?= !$stockBadge['available'] ? 'disabled' : '' ?>
                                                onclick="event.preventDefault(); event.stopPropagation(); addToCart('<?= $productId ?>')">
                                                <svg class="w-4 h-4 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                                                </svg>
                                            </button>
                                            <button type="button" class="flex-1 rounded-lg bg-primary px-3 py-2.5 text-sm font-medium text-white transition-all hover:bg-[#A14646] hover:shadow-md <?= !$stockBadge['available'] ? 'opacity-50 cursor-not-allowed' : '' ?>" <?= !$stockBadge['available'] ? 'disabled' : '' ?>
