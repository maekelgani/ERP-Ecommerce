...
$result = $orderRepo->getAllOrders($filters, $page, $limit);
$orders = $result['data'];
...
<body class="bg-gray-50 h-screen flex">
    ...
    <div class="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-6">
            <div class="mb-6">
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Semua Pesanan</h1>
                <p class="text-gray-500 mt-1">Lihat dan kelola semua riwayat pesanan</p>
            </div>
                    ...
                    <div class="flex flex-wrap items-end gap-3">
                        ...
                        <div class="w-full md:w-auto">
                            ...
                            <select name="payment_status" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#882426]/20 focus:border-[#882426] outline-none">
                                ...
                                <option value="gagal" <?= $filters['payment_status'] === 'gagal' ? 'selected' : '' ?>>Gagal</option>
                            </select>
                        </div>
                        <div class="w-full md:w-auto">
                        ...
                        <div class="flex gap-2">
                            ...
                            <a href="OrderAdmin.php" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition">
                                Reset
                            </a>
                        </div>
                    </div>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-100">
                                ...
                                <th class="px-5 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Tanggal Order</th>
                                <th class="px-5 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Metode Bayar</th>
                                ...
                                foreach ($orders as $order): ?>
                                    <tr class="hover:bg-orange-50 transition-colors">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-2">
                                                <?php
                                                $methodIcon = match ($order['metode_pembayaran'] ?? '') {
                                                    'transfer_bank' => 'account_balance',
                                                    'ewallet' => 'wallet',
                                                    'cod' => 'payments',
                                                ...
                                                ?>
                                                <span class="material-symbols-outlined text-gray-400 text-lg"><?= $methodIcon ?></span>
                                                <div>
                                                    <p class="text-sm text-gray-800"><?= getPaymentMethodLabel($order['metode_pembayaran']) ?></p>
                                                    <?php if ($order['nama_bank']): ?>
                                                        <p class="text-xs text-gray-500"><?= htmlspecialchars(strtoupper($order['nama_bank'])) ?></p>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            <?= getStatusOrderBadge($order['status_order']) ?>
                                        </td>
                                        <td class="px-4 py-3">
                                            <div class="flex items-center justify-center gap-1">
                                                ...
                                                <?php if ($order['status_order'] !== 'selesai' && $order['status_order'] !== 'dibatalkan'): ?>
                                                    <button class="btn-update-status inline-flex items-center gap-1.5 px-3 py-2 text-green-600 bg-green-50 rounded-lg text-sm font-medium hover:bg-green-100 transition-colors"
                                                        data-order-id="<?= htmlspecialchars($order['id_order']) ?>"
                                                        data-current-status="<?= htmlspecialchars($order['status_order']) ?>"
                                                        title="Update Status">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden transform transition-all animate-modal-in">
                ...
                <div class="bg-[#882426] px-6 py-5 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-xl font-bold text-white">Update Status Order</h3>
                            <p class="text-white/70 text-sm mt-0.5">Ubah status pesanan pelanggan</p>
                        </div>
                    </div>
                    <button onclick="closeStatusModal()" class="w-10 h-10 rounded-xl bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all duration-200">
                        <span class="material-symbols-outlined">close</span>
                    </button>
                </div>
                ...
                <div class="p-6">
                    ...
                    <div class="mb-5 p-4 rounded-xl border-l-4 border-[#882426] bg-[#882426]/5">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="text-sm text-gray-700 font-medium">Perubahan status akan langsung diperbarui dan notifikasi akan dikirim ke customer.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form id="status-form" class="space-y-5">
                        ...
                        <div>
                            ...
                            <select name="status" id="status-select" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#882426]/20 focus:border-[#882426] transition-all duration-200">
                            ...
                            <p class="text-xs text-gray-500 mt-2 flex items-center gap-1">
                                ...
                                Customer akan menerima notifikasi perubahan status
                            </p>
                        </div>
